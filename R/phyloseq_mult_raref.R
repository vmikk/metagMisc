...
#'
#' @details
#' If the sample size ('SampSize') is not specified, rarefaction will be made for the depth equal to the minimal observed sample size.
...
#' 
#' To perform multiple rarefactions and estimate diversity, use \code{\link{phyloseq_mult_raref_div}}.
#'
#' @return List of rarefied phyloseq-objects.
#' @seealso \code{\link[phyloseq]{rarefy_even_depth}}, \code{\link{phyloseq_mult_raref_div}}
#' @export
#'
#' @examples
#' library(phyloseq)
...
#'
phyloseq_mult_raref <- function(x,
  SampSize = min(phyloseq::sample_sums(x)), 
